...
`references/` is the shared pool for all four roblox-optimum skills — **code-review**, **diagnose**, and **studio-ops** read these same files rather than carrying copies that would drift. Renaming or removing one breaks a sibling skill, so `scripts/audit.mjs` fails the build on any cross-skill link that leaves this pool.
...
|---|---|
| **Supervision level** — invocation argument `ask`/`bal`/`go`, or `!ask`/`!bal`/`!go` inline | The configured level `${user_config.supervision}` if that names one of `ask`, `bal`, or `go`, otherwise **Balanced**. Never ask which level the user wants — absence *is* the answer |
...
- **Detect the project environment first** — **Studio-native**, or a filesystem project synced by Script Sync, Rojo, Argon, or Azul. **Which side is the source of truth differs per tool**, and assuming wrong overwrites the user's work. Never start, stop, or reconfigure a sync session unasked, and on an MCP connection run the preflight before any write. The detail lives in the **studio-ops** skill; when a task turns on which side wins, or on proving a change works in a running session, that skill owns it.
