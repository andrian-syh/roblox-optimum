...
5. **Weekly updates pages — what changed and whether it is live yet.** Roblox now publishes engine changes as one page per week at `https://create.roblox.com/docs/updates/<YYYY-MM-DD>`, dated to the Monday of that week, each entry carrying a **Status** column and a `Previous` link chaining backwards through earlier weeks. **Append `.md` and these fetch as raw markdown**, so this is the cheapest read of recent changes — walk the chain back from the current week when catching up. One trap: a dated page's own tables render client-side, so fetch the `.md` and not the HTML. The sibling page `https://create.roblox.com/docs/updates/pending` lists changes **not yet live** and has no `.md` variant — `pending.md` returns 404 — but its entries are not lost: fetch the HTML and read the JSON in its `<script id="__NEXT_DATA__">` tag, where `props.pageProps.data.pendingReleaseContents` is the list. Nothing there is live, so it settles what is coming, never what exists. `devforum.roblox.com/c/updates/release-notes` still carries the same notes as server-rendered topics, plus the feature-flag list and developer replies. Either way a release note announces intent — a name from one is provisional until step 2 confirms it.
...
**Luau 0.736 and 0.737 added no language feature, syntax, or library function a script author can call.** Both are compiler, type-solver, and JIT work: lambda generalization in table literals, fewer internal compiler errors, better cyclic-require messages, `keyof` unions ordered lexicographically, and an upvalue fix for `continue` inside `repeat..until`. Recorded so the next maintainer does not re-read them looking for a row to add.
