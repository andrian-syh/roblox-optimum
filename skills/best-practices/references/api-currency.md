...
A previous snapshot of this file claimed "737 is one Studio flag-override improvement and one layered-clothing fix." That was wrong, and the way it was wrong is worth keeping. It read a *weekly* page, which lists only what is already live, and concluded the version itself was thin. The versioned page for 737 carries **22 entries**, of which 4 were live; 736 carries 17, 738 carries 26, and 739 carries 21. The layered-clothing fix belongs to 736, not 737. **Never size a version from a weekly page** — step 5 below says which page answers which question. Nothing in the pending list is a deprecation, but the API dump tagged four `GuiObject` members deprecated at v738 ahead of the published index; see [Deprecated](#deprecated-report-as-findings). The performance figures and tooling in [performance.md](performance.md), [device-performance.md](device-performance.md), and [verification.md](verification.md) were read directly from the Creator Hub performance-optimization guides (`create.roblox.com/docs/performance-optimization`, its `design`, `identify`, `improve`, `monitor`, `scene-analysis`, `test-on-hardware`, and `microprofiler/*` pages) and the `Workspace` class reference.
...
| ~~`PlayerControlState`~~ → `ControlState` + `StateSchema` | **[Undocumented]** | `PlayerControlState` was added at **v735** and **removed at v738** (API dump). Its replacement pair arrived in the same version: `ControlState` (`Owner`, `GetState`, `GetChangedState`, `SetField`, `UpdateFields`, `GetReplicationWeight`, `OnStateChanged`) and `StateSchema` (`AddBoolField`, `AddCFrameField`, `AddInstanceField`, `AddIntField`, `AddNumberField`, `AddUnitVector3Field`, `AddVector2Field`, `AddVector3Field`). Replicates scripted state under Server Authority ([server-authority.md](server-authority.md)). No reference page for either — probe in Studio before designing around them, and **do not write `PlayerControlState`**, which no longer exists |
...
| `Workspace.StreamingAdaptiveRadius`, `Workspace.UseInputSink` | **[Verify]** | Both added in engine **v738** (API dump). Scriptability unconfirmed for either; the dump marks them alongside the Studio-only streaming properties, so treat them as settings to evaluate rather than assignments to write ([device-performance.md](device-performance.md#engine-levers-before-script-levers)) |
| `WorldRoot:Simulate()`, `.AutoSimulate`, `.SimulationRate`, `.GravityDirection`, `.Wind`, `.WindDirection` | **[Undocumented]** | Added in engine **v737**–**v738** (API dump; announced in release notes 737). Scriptable physics stepping: `Simulate()` steps a world manually, `AutoSimulate` controls whether it steps itself, `SimulationRate` scales the rate. Makes a `WorldModel` usable for deterministic offline simulation. No reference page — probe before relying on step semantics |
...
| `GuiObject.InputSink` (`Enum.InputSink`), and the new `GuiObject.Sink` | **[Verify]** | Absorbs input at the element, replacing `Active`-style input blocking. **A rename is in progress.** At **v738** the dump changed `InputSink` to `[NotReplicated]` with `CanLoad`/`CanSave` both `false`, and added a `Sink` property alongside it; `Workspace.UseInputSink` arrived in the same version. `InputSink` is still the one the Engine API Reference documents and still works at runtime, but it no longer serializes, so a value set in Studio will not survive a save. `Sink` has no reference page and this skill has **not** confirmed its security level or value type — one Edit-mode probe settles both. Until then write `InputSink`, and never flag either name as nonexistent |
...
**The failure this file has actually suffered, twice.** A previous pass treated *absent from the Engine API Reference* as *does not exist*, and on that basis declared `GuiService:GetUIScaleMultiplier`, `UIShadow.Mode`, and `UIShadow.Inset`/`ShowBehindParent` to be release-note names that never shipped. All of them had shipped — v734, v732, and v733 respectively — and the claim propagated into `style-rules.md` and `false-positives.md` as an instruction to **flag correct code**. (`GetUIScaleMultiplier` was later genuinely removed, at v738. That does not retroactively make the old claim right: it was unsupported when it was made, and the dump is what settled both its existence then and its removal now.) Five further rows were held at [Verify] for the same reason while already present in the dump.
