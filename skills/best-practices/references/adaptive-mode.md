...
If the environment supports project memory or a conventions file (e.g., `CLAUDE.md`, `CONVENTIONS.md`), offer to save the confirmed convention there so future sessions skip re-analysis. Keep it as a diff against this skill's defaults ("same as roblox-optimum:best-practices except: ...") rather than a full copy.
