...
**Adaptive streaming radius** (`Workspace.StreamingAdaptiveRadius`) lets the engine move the radius itself in response to the client's conditions, instead of holding the fixed `StreamingMinRadius`/`StreamingTargetRadius` pair above for every device. It is newer than the recommended values in that table and this skill has **not** confirmed its scriptability or value set ([api-currency.md](api-currency.md#engine)); treat it as a Studio-side setting to evaluate, not as an assignment to write, and measure on a low-end device before preferring it to the fixed radii.
