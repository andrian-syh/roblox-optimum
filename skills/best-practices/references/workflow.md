...
|---|---|---|---|
| **Supervision level** | Invocation argument (`/roblox-optimum:best-practices ask`) > inline token (`!ask`/`!bal`/`!go`) > session declaration in any words ("watch every step", "stop asking, just go") > default | **Balanced** — never ask which level the user wants; absence *is* the answer | [below](#supervision-levels) |
